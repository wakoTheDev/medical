...
$patientNotes = isset($_POST['patient_notes']) ? $_POST['patient_notes'] : '';
$doctorId = $_SESSION['userId'];

// Validate data
if ($id <= 0 || empty($appointmentTime) || empty($status)) {
...
// Update the booking
$stmt = $conn->prepare("UPDATE doctor_visits SET appointment_time = ?, status = ?, patient_notes = ?, updated_at = NOW() WHERE id = ? AND doctor_id = ?");
$stmt->bind_param("sssii", $appointmentTime, $status, $patientNotes, $id, $doctorId);
